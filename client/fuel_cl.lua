...
if Config.FuelDebug then
	RegisterCommand('setfuel', function(source, args)
		if args[1] == nil then
			print("You forgot to put a fuel level!")
			return
		end
...
function GetClosestPump(coords, isElectric)
	if isElectric then
		...
		electricPump       = GetClosestObjectOfType(coords.x, coords.y, coords.z, 3.0, joaat("electric_charger"), true,
			true,
			true)
		...
		for i = 1, #props, 1 do
			...
			pump                   = GetClosestObjectOfType(coords.x, coords.y, coords.z, 3.0, joaat(currentPumpModel),
				true, true, true)
			...
			for _, v in pairs(result) do
				...
				if info == "all" or info == "reserves" then
					...
					if Config.UnlimitedFuel then
						...
						if Config.FuelDebug then
							print(
								"Reserves are not buyable, because Config.UnlimitedFuel is set to true.")
						end
					end
				end
				-- Fuel Price --
...
local function CanAfford(price, purchasetype)
	...
	elseif purchasetype == 'cash' then
		Money =
		    QBCore.Functions.GetPlayerData().money['cash']
	end
	if Money < price then
		return false
	else
...
if Config.ShowNearestGasStationOnly then
		...
		while true do
			...
			for _, ourCoords in pairs(Config.GasStations) do
				...
				if not (location > #Config.GasStations) then -- Make sure we are not going over the amount of locations available.
					local gasStationCoords = vector3(Config.GasStations[location].pedcoords.x,
						Config.GasStations[location].pedcoords.y, Config.GasStations[location].pedcoords.z)
	...
	RegisterNetEvent('cdn-fuel:client:updatestationlabels', function(location, newLabel)
		...
		if Config.FuelDebug then print("Changing Label for Location #" .. location .. ' to ' .. newLabel) end
		Config.GasStations[location].label = newLabel
		local coords                       = vector3(Config.GasStations[location].pedcoords.x,
			Config.GasStations[location].pedcoords.y,
			Config.GasStations[location].pedcoords.z)
	...
	CreateThread(function()
		...
		for i = 1, #Config.GasStations, 1 do
			local location            = i
			gasStationCoords          = vector3(Config.GasStations[location].pedcoords.x,
				Config.GasStations[location].pedcoords.y, Config.GasStations[location].pedcoords.z)
...
if Config.RenewedPhonePayment then
	RegisterNetEvent('cdn-fuel:client:phone:PayForFuel', function(amount)
			...
			if shouldRecieveDiscount then
				...
				if discount ~= 0 then
					if discount == 100 then
						...
						FuelPrice       = 0
						if Config.FuelDebug then
							print("Your discount for Emergency Services is set @ " ..
								discount .. "% so fuel is free!")
						end
					else
						...
						if Config.FuelDebug then
							print("Your discount for Emergency Services is set @ " ..
								discount .. "%. Setting new price to: $" .. FuelPrice)
						end
					end
				else
					if Config.FuelDebug then
						print("Your discount for Emergency Services is set @ " ..
							discount .. "%. It cannot be 0 or < 0!")
					end
				end
			end
		end
		...
		local success = exports['qb-phone']:PhoneNotification(Lang:t("fuel_phone_header"),
			Lang:t("phone_notification") .. total, 'fas fa-gas-pump', '#9f0e63', "NONE", 'fas fa-check-circle',
			'fas fa-times-circle')
		if success then
...
if Config.Ox.Menu then
	RegisterNetEvent('cdn-fuel:client:OpenContextMenu', function(total, fuelamounttotal, purchasetype)
		...
		lib.registerContext({
			...
			title   = Lang:t("menu_purchase_station_header_1") ..
...
RegisterNetEvent('cdn-fuel:client:grabnozzle', function()
	if Config.PlayerOwnedGasStationsEnabled then
		...
		QBCore.Functions.TriggerCallback('cdn-fuel:server:checkshutoff', function(result)
			if result == true then
				QBCore.Functions.Notify(Lang:t("emergency_shutoff_active"), 'error', 7500)
				ShutOff = true
				return
			else
				ShutOff = false
	...
	if not ShutOff then
		...
		if Config.PumpHose then
			...
			if PumpHeightAdd == nil then
				PumpHeightAdd = 2.1
				if Config.FuelDebug then
					print("PumpHeightAdd was not configured for location: #" ..
						CurrentLocation .. " so, we are defaulting to 2.0!")
				end
			end
			...
			if Config.FuelDebug then
				...
				print(Rope, pump, fuelnozzle, pumpCoords.x, pumpCoords.y, pumpCoords.z, nozzlePos.x, nozzlePos.y,
					nozzlePos.z, length)
			...
			while holdingnozzle do
				...
				if dist > 7.5 then
					...
					if Config.FuelNozzleExplosion then
						AddExplosion(grabbednozzlecoords.x, grabbednozzlecoords.y, grabbednozzlecoords.z,
							'EXP_TAG_PROPANE', 1.0, true, false, 5.0)
						StartScriptFire(grabbednozzlecoords.x, grabbednozzlecoords.y, grabbednozzlecoords.z - 1, 25,
							false)
						SetFireSpreadRate(10.0)
						Wait(5000)
						StopFireInRange(grabbednozzlecoords.x, grabbednozzlecoords.y, grabbednozzlecoords.z - 1,
							3.0)
					end
				end
				Wait(2500)
	...
	if RefuelingType == nil then
		...
		if Config.PlayerOwnedGasStationsEnabled and not Config.UnlimitedFuel then
			if ReserveLevels < 1 then
				QBCore.Functions.Notify(Lang:t("station_no_fuel"), 'error', 7500)
				return
			end
		end
		if Config.PlayerOwnedGasStationsEnabled then
		...
		if shouldRecieveDiscount then
			...
			if discount ~= 0 then
				if discount == 100 then
					...
					discount = discount / 100
					if Config.FuelDebug then
						print("Math( Current Fuel Price: " ..
							FuelPrice .. " - " .. FuelPrice * discount .. "<<-- FuelPrice * Discount)")
					end
					CachedFuelPrice = FuelPrice
					FuelPrice       = (FuelPrice) - (FuelPrice * discount)
					if Config.FuelDebug then
				...
				if Config.FuelDebug then
					print("Your discount for Emergency Services is set @ " .. discount ..
						"%. It cannot be 0 or < 0!")
				end
			end
		end
	...
	if Config.Ox.Input then
		if Config.PlayerOwnedGasStationsEnabled and not Config.UnlimitedFuel and not RefuelingType == 'special' then
			if ReserveLevels < maxfuel then
				...
				if not fuel then
					if Config.FuelDebug then print("Fuel Is Nil! #1") end
					return
				end
				...
				if not fuel then
					if Config.FuelDebug then print("Fuel Is Nil! #2") end
					return
				end
			...
			if not fuel then
				if Config.FuelDebug then print("Fuel Is Nil! #3") end
				return
			end
			fuelAmount = tonumber(fuel[4])
		end
		if fuel then
			if not fuelAmount then
				print("Fuel Amount Nil")
				return
			end
			if not holdingnozzle and RefuelingType ~= 'special' then
				QBCore.Functions.Notify(Lang:t("no_nozzle"), 'error')
				return
			end
			if Config.PlayerOwnedGasStationsEnabled and not Config.UnlimitedFuel and not RefuelingType == "special" then
				if tonumber(fuelAmount) > tonumber(ReserveLevels) then
					QBCore.Functions.Notify(Lang:t("station_not_enough_fuel"), "error")
					return
				end
			end
			...
			if ReserveLevels < maxfuel then
				...
				fuel                       = exports['qb-input']:ShowInput({
					header     = "Select the Amount of Fuel<br>Current Price: $" ..
					    FuelPrice ..
					    " / Liter <br> Current Fuel: " .. finalfuel .. " Liters <br> Full Tank Cost: $" ..
		...
		if fuel then
			if not fuel.amount then
				if Config.FuelDebug then print("fuel.amount  = nil") end
				return
			end
			if not holdingnozzle and RefuelingType ~= 'special' then
				QBCore.Functions.Notify(Lang:t("no_nozzle"))
				return
			end
			if Config.PlayerOwnedGasStationsEnabled and not Config.UnlimitedFuel and not RefuelingType == 'special' then
			...
			if (fuel.amount + finalfuel) >= 100 then
				...
				if GlobalTax(fuel.amount * FuelPrice) + (fuel.amount * FuelPrice) <= money then
					if Config.FuelDebug then
						print("Player is getting " ..
							fuel.amount ..
							"L of Fuel @ " ..
							FuelPrice ..
							'/L, Total Cost: ' .. GlobalTax(fuel.amount * FuelPrice) + (fuel.amount * FuelPrice))
					end
					TriggerServerEvent('cdn-fuel:server:OpenMenu', fuel.amount, inGasStation, false, purchasetype,
						tonumber(FuelPrice))
				else
					QBCore.Functions.Notify(Lang:t("not_enough_money"), 'error', 7500)
				end
		...
		if IsPlayerNearVehicle() then
				...
				while refueling do
					...
					if Cancelledrefuel then
						...
						local refillCost        = (finalrefuelamount * FuelPrice) +
						    GlobalTax(finalrefuelamount * FuelPrice)
						if Config.RenewedPhonePayment and purchasetype == "bank" then
							...
							TriggerServerEvent('cdn-fuel:server:PayForFuel', refillCost, purchasetype, FuelPrice,
								false, CachedFuelPrice)
							CachedFuelPrice = nil
						end
						if RefuelingType == nil then
							if Config.PlayerOwnedGasStationsEnabled and not Config.UnlimitedFuel then
								TriggerServerEvent('cdn-fuel:station:server:updatereserves', "remove",
									finalrefuelamount, ReserveLevels, CurrentLocation)
								if CachedFuelPrice ~= nil then
									if Config.FuelDebug then
										print("We have a cached price: $" ..
											CachedFuelPrice .. ", we will credit this to the gas station.")
									end
									TriggerServerEvent('cdn-fuel:station:server:updatebalance', "add",
										finalrefuelamount, StationBalance, CurrentLocation, CachedFuelPrice)
									CachedFuelPrice = nil
								else
									TriggerServerEvent('cdn-fuel:station:server:updatebalance', "add",
										finalrefuelamount, StationBalance, CurrentLocation, FuelPrice)
								end
							end
						end
						local curfuel   = GetFuel(vehicle)
			...
			if Config.Ox.Progress then
				if lib.progressCircle({
					...
					if RefuelingType == nil then
						if Config.PlayerOwnedGasStationsEnabled and not Config.UnlimitedFuel then
							TriggerServerEvent('cdn-fuel:station:server:updatereserves', "remove", fuelamount,
							...
							if CachedFuelPrice ~= nil then
								...
								TriggerServerEvent('cdn-fuel:station:server:updatebalance', "add", fuelamount,
							...
							if Config.FuelDebug then
								print(
									"Config.PlayerOwnedGasStationsEnabled == false or Config.UnlimitedFuel == true, this means reserves will not be changed.")
							end
						end
						if Config.FuelDebug then
							print(
					...
					StopAnimTask(ped, Config.RefuelAnimationDictionary, Config.RefuelAnimation, 3.0, 3.0, -1, 2, 0,
						0, 0, 0)
...
RegisterNetEvent('cdn-fuel:jerrycan:refuelmenu', function(itemData)
	if IsPedInAnyVehicle(PlayerPedId(), false) then
		QBCore.Functions.Notify(Lang:t("cannot_refuel_inside"), 'error')
		return
	end
	...
	if GetVehicleBodyHealth(vehicle) < 100 then
		QBCore.Functions.Notify(Lang:t("vehicle_is_damaged"), 'error')
		return
	end
...
RegisterNetEvent('cdn-fuel:client:purchasejerrycan', function()
	...
	if Config.Ox.Menu then
		...
		exports['qb-menu']:openMenu({
			{
				header       = Lang:t("menu_jerry_can_purchase_header") ..
				    (math.ceil(Config.JerryCanPrice + GlobalTax(Config.JerryCanPrice))),
				isMenuHeader = true,
		...
		if Config.Ox.Input then
			...
			if refuel and refuelAmount then
				if tonumber(refuelAmount) == 0 then
					QBCore.Functions.Notify(Lang:t("more_than_zero"), 'error')
					return
				...
				if tonumber(refuelAmount) > jerrycanfuelamount then
					QBCore.Functions.Notify(Lang:t("jerry_can_not_enough_fuel"), 'error')
					return
				end
				...
				local lefthand     = GetPedBoneIndex(ped, 18905)
				AttachEntityToEntity(JerrycanProp, ped, lefthand, 0.11 --[[Left - Right (Kind of)]],
					0.0 --[[Up - Down]], 0.25 --[[Forward - Backward]], 15.0, 170.0, 90.42, 0, 1, 0, 1, 0, 1)
				if Config.Ox.Progress then
					if lib.progressCircle({
					...
					QBCore.Functions.Progressbar('refuel_gas', Lang:t("prog_refueling_vehicle"), refueltimer, false,
						true,
				...
				inputs     = {
					{
						...
						isRequired = true,
						name       = 'amount',
						text       = Lang:t("input_max_fuel_footer_1") ..
						    maxvehrefuel .. Lang:t("input_max_fuel_footer_2")
					}
				}
			})
			if refuel then
				if tonumber(refuel.amount) == 0 then
					QBCore.Functions.Notify(Lang:t("more_than_zero"), 'error')
					return
				elseif tonumber(refuel.amount) < 0 then
					QBCore.Functions.Notify(Lang:t("more_than_zero"), 'error')
					return
				end
				if tonumber(refuel.amount) > jerrycanfuelamount then
					QBCore.Functions.Notify(Lang:t("jerry_can_not_enough_fuel"), 'error')
					return
				end
				...
				if vehfuel + tonumber(refuel.amount) > 100 then
					QBCore.Functions.Notify(Lang:t("tank_cannot_fit"), 'error')
					return
				end
				JerrycanProp   = CreateObject(joaat('w_am_jerrycan'), 1.0, 1.0, 1.0, true, true, false)
				...
				if Config.Ox.Progress then
					if lib.progressCircle({
						...
						local JerryCanItemData = data.itemData
						local srcPlayerData    = QBCore.Functions.GetPlayerData()
						TriggerServerEvent('cdn-fuel:info', "remove", tonumber(refuel.amount), srcPlayerData,
							JerryCanItemData)
	...
	if Config.Ox.Input then
		...
		if refuel then
			...
			if tonumber(refuelAmount) + tonumber(jerrycanfuelamount) > Config.JerryCanCap then
				QBCore.Functions.Notify(Lang:t("jerry_can_not_fit_fuel"), 'error')
				return
			end
		...
		local refuel            = exports['qb-input']:ShowInput({
			...
			inputs     = { {
				...
				name       = 'amount',
				text       = Lang:t("input_max_fuel_footer_1") ..
				    JerryCanMaxRefuel .. Lang:t("input_max_fuel_footer_2")
			} }
		})
		if refuel then
			if tonumber(refuel.amount) == 0 then
			...
			if tonumber(refuel.amount) + tonumber(jerrycanfuelamount) > Config.JerryCanCap then
				QBCore.Functions.Notify(Lang:t("jerry_can_not_fit_fuel"), 'error')
				return
			end
			if tonumber(refuel.amount) > Config.JerryCanCap then
				QBCore.Functions.Notify(Lang:t("jerry_can_not_fit_fuel"), 'error')
				return
			end
			local refueltimer = Config.RefuelTime * tonumber(refuel.amount)
			...
			AttachEntityToEntity(JerrycanProp, ped, lefthand, 0.11 --[[Left - Right]], 0.05 --[[Up - Down]],
				0.27 --[[Forward - Backward]], -15.0, 170.0, -90.42, 0, 1, 0, 1, 0, 1)
			SetEntityVisible(fuelnozzle, false, 0)
			QBCore.Functions.Progressbar('refuel_gas', Lang:t("prog_jerry_can_refuel"), refueltimer, false, true,
				{ -- Name | Label | Time | useWhileDead | canCancel
					disableMovement    = true,
					disableCarMovement = true,
					disableMouse       = false,
					...
					if Config.Ox.Inventory then
						TriggerServerEvent('cdn-fuel:info', "add", tonumber(refuelAmount), srcPlayerData,
							jerryCanData)
					else
						TriggerServerEvent('cdn-fuel:info', "add", tonumber(refuelAmount), srcPlayerData,
							jerryCanData)
					...
					if RefuelingType == nil then
						if Config.PlayerOwnedGasStationsEnabled and not Config.UnlimitedFuel then
							TriggerServerEvent('cdn-fuel:station:server:updatereserves', "remove",
								tonumber(refuel.amount), ReserveLevels, CurrentLocation)
							if CachedFuelPrice ~= nil then
								TriggerServerEvent('cdn-fuel:station:server:updatebalance', "add",
									tonumber(refuel.amount), StationBalance, CurrentLocation, CachedFuelPrice)
							else
								TriggerServerEvent('cdn-fuel:station:server:updatebalance', "add",
									tonumber(refuel.amount), StationBalance, CurrentLocation, FuelPrice)
							end
						else
							if Config.FuelDebug then
								print(
					...
					local total = (tonumber(refuel.amount) * FuelPrice) +
					    GlobalTax(tonumber(refuel.amount) * FuelPrice)
					TriggerServerEvent('cdn-fuel:server:PayForFuel', total, "cash", FuelPrice, false,
						CachedFuelPrice)
				end, function() -- Play When Cancel
					SetEntityVisible(fuelnozzle, true, 0)
					DeleteObject(JerrycanProp)
	...
	if chance < Config.SyphonPoliceCallChance then
		...
		elseif Config.SyphonDispatchSystem == "custom" then
			...
			if Config.SyphonDebug then
				print(
					"There was an attempt to call police but this dispatch system is not supported!")
			end
		end
	end
end
...
RegisterNetEvent('cdn-syphoning:syphon:menu', function(itemData)
	if IsPedInAnyVehicle(PlayerPedId(), false) then
		QBCore.Functions.Notify(Lang:t("syphon_inside_vehicle"), 'error')
		return
	end
	...
	if Config.ElectricVehicleCharging then
		...
		if Config.ElectricVehicles[vehiclename] and Config.ElectricVehicles[vehiclename].isElectric then
			...
			if Config.SyphonDebug then
				print("^2" ..
					current ..
					"^5 has been found. It ^2matches ^5the Player's Vehicle: ^2" ..
					vehiclename .. ". ^5This means syphoning will not be allowed.")
			end
			QBCore.Functions.Notify(Lang:t("syphon_electric_vehicle"), 'error', 7500)
			return
		end
	else
		NotElectric = true
	...
	if NotElectric then
		...
		if HasSyphon then
			...
			if reason == "syphon" then
				if Config.Ox.Input then
					syphon = lib.inputDialog('Begin Syphoning',
					...
					if syphon then
						...
						if tonumber(syphonAmount) < 0 then
							QBCore.Functions.Notify(Lang:t("syphon_more_than_zero"), 'error')
							return
						end
						if tonumber(syphonAmount) == 0 then
							QBCore.Functions.Notify(Lang:t("syphon_more_than_zero"), 'error')
						...
						if tonumber(syphonAmount) > maxsyphon then
							QBCore.Functions.Notify(
								Lang:t("syphon_kit_cannot_fit_1") ..
								fitamount .. Lang:t("syphon_kit_cannot_fit_2"),
								'error')
							return
						end
						...
						if (tonumber(syphonAmount) <= tonumber(cargasamount)) then
							...
							if lib.progressCircle({
								...
								if GetFuel(vehicle) >= syphonAmount then
									...
									local srcPlayerData = QBCore.Functions.GetPlayerData()
									TriggerServerEvent('cdn-fuel:info', "add", tonumber(syphonAmount),
										srcPlayerData, syphonData)
								else
					...
					if syphon then
						...
						if tonumber(syphon.amount) < 0 then
							QBCore.Functions.Notify(Lang:t("syphon_more_than_zero"), 'error')
							return
						end
						if tonumber(syphon.amount) == 0 then
						...
						if tonumber(syphon.amount) > maxsyphon then
							QBCore.Functions.Notify(
								Lang:t("syphon_kit_cannot_fit_1") ..
								fitamount .. Lang:t("syphon_kit_cannot_fit_2"),
								'error')
						...
						if currentsyphonamount + syphon.amount > Config.SyphonKitCap then
							QBCore.Functions.Notify(
								Lang:t("syphon_kit_cannot_fit_1") ..
								fitamount .. Lang:t("syphon_kit_cannot_fit_2"),
								'error')
						...
						if (tonumber(syphon.amount) <= tonumber(cargasamount)) then
							...
							QBCore.Functions.Progressbar('syphon_gas', Lang:t("prog_syphoning"), syphontimer,
								false, true,
								{ -- Name | Label | Time | useWhileDead | canCancel
									disableMovement    = true,
									disableCarMovement = true,
									...
									if GetFuel(vehicle) >= tonumber(syphon.amount) then
										...
										local syphonData    = data.itemData
										local srcPlayerData = QBCore.Functions.GetPlayerData()
										TriggerServerEvent('cdn-fuel:info', "add", tonumber(syphon.amount),
											srcPlayerData, syphonData)
										StopAnimTask(ped, Config.StealAnimDict, Config.StealAnim, 1.0)
									else
										QBCore.Functions.Notify(Lang:t("menu_syphon_vehicle_empty"), 'error')
			...
			elseif reason == "refuel" then
				...
				if Config.Ox.Input then
					refuel = lib.inputDialog(Lang:t("input_select_refuel_header"),
					...
					if refuel then
						...
						elseif tonumber(refuelAmount) > 100 then
							QBCore.Functions.Notify("You can't refuel more than 100L!", 'error')
							return
						end
						if tonumber(refuelAmount) > tonumber(currentsyphonamount) then
							QBCore.Functions.Notify(Lang:t("syphon_not_enough_gas"), 'error')
							return
						end
						...
						inputs     = {
							{
								...
								name       = 'amount',
								text       = Lang:t("input_max_fuel_footer_1") ..
								    Maxrefuel .. Lang:t("input_max_fuel_footer_2")
							}
						}
					})
					if refuel then
						...
						elseif tonumber(refuel.amount) < 0 then
							QBCore.Functions.Notify(Lang:t("more_than_zero"), 'error')
							return
						elseif tonumber(refuel.amount) > 100 then
							QBCore.Functions.Notify("You can't refuel more than 100L!", 'error')
							return
						end
						if tonumber(refuel.amount) > tonumber(currentsyphonamount) then
							QBCore.Functions.Notify(Lang:t("syphon_not_enough_gas"), 'error')
							return
						end
						if tonumber(refuel.amount) + tonumber(cargasamount) > 100 then
							QBCore.Functions.Notify(Lang:t("tank_cannot_fit"), 'error')
							return
						end
						local refueltimer = Config.RefuelTime * tonumber(refuel.amount)
						if tonumber(refuel.amount) < 10 then refueltimer = Config.RefuelTime * 10 end
						QBCore.Functions.Progressbar('refuel_gas', Lang:t("prog_refueling_vehicle"), refueltimer,
							false, true,
							{ -- Name | Label | Time | useWhileDead | canCancel
								disableMovement    = true,
								disableCarMovement = true,
								...
								local syphonData    = data.itemData
								local srcPlayerData = QBCore.Functions.GetPlayerData()
								TriggerServerEvent('cdn-fuel:info', "remove", tonumber(refuel.amount),
									srcPlayerData,
									syphonData)
							end, function() -- Play When Cancel
								StopAnimTask(ped, Config.JerryCanAnimDict, Config.JerryCanAnim, 1.0)
...
RegisterNetEvent('cdn-fuel:client:grabnozzle:special', function()
	...
	if Config.PumpHose then
		...
		if Config.FuelDebug then
			print("Hose Properties:")
			print(Rope, pump, SpecialFuelNozzleObj, pumpCoords.x, pumpCoords.y, pumpCoords.z, nozzlePos.x, nozzlePos
				.y, nozzlePos.z, length)
	...
	if resource == GetCurrentResourceName() then
		if LocalPlayer.state['isLoggedIn'] then
			for i = 1, #Config.AirAndWaterVehicleFueling['locations'], 1 do
				...
				local minimumZ, maximumZ      = currentLocation['PolyZone']['minmax']['min'],
				    currentLocation['PolyZone']['minmax']['max']
				...
				AirSeaFuelZones[k].PolyZone:onPlayerInOut(function(isPointInside)
					if isPointInside then
						...
						if Config.AirAndWaterVehicleFueling['locations'][i]['whitelist']['enabled'] then
							local whitelisted_jobs = Config.AirAndWaterVehicleFueling['locations'][i]
							    ['whitelist']['whitelisted_jobs']
							local plyJob           = QBCore.Functions.GetPlayerData().job

							if Config.FuelDebug then
								print("Player Job: " .. plyJob.name ..
									" Is on Duty?: " .. json.encode(plyJob.onduty))
							end
						...
						if canUseThisStation then
							...
							local DrawText          = Config.AirAndWaterVehicleFueling['locations'][i]
							    ['draw_text']
								...
								while PlayerInSpecialFuelZone do
									...
									if IsControlJustReleased(0, Config.AirAndWaterVehicleFueling['refuel_button']) --[[ Control in Config ]] then
										...
										elseif IsPedInAnyVehicle(PlayerPedId(), true) then
											...
											if Config.FuelDebug then
												print(
													"Attempting to Open Fuel menu for special vehicles.")
											end
				...
				if currentLocation['prop'] then
					...
					AirSeaFuelZones[k].prop = CreateObject(model, modelCoords.x, modelCoords.y, modelCoords.z, false,
						true, true)
					...
					if Config.FuelDebug then
						print("Location #" ..
							i ..
							" for Special Fueling Zones (Air and Sea) doesn't have a prop set up, so players cannot fuel here.")
					end
				end
	...
	for i = 1, #Config.AirAndWaterVehicleFueling['locations'], 1 do
		...
		AirSeaFuelZones[k].PolyZone:onPlayerInOut(function(isPointInside)
			if isPointInside then
				local canUseThisStation = false
				if Config.AirAndWaterVehicleFueling['locations'][i]['whitelist']['enabled'] then
					local whitelisted_jobs = Config.AirAndWaterVehicleFueling['locations'][i]['whitelist']
					    ['whitelisted_jobs']
					local plyJob           = QBCore.Functions.GetPlayerData().job
		...
		if currentLocation['prop'] then
			...
			local modelCoords       = currentLocation['prop']['coords']
			local heading           = modelCoords[4] - 180.0
			AirSeaFuelZones[k].prop = CreateObject(model, modelCoords.x, modelCoords.y, modelCoords.z, false, true,
				true)
			if Config.FuelDebug then print("Created Special Pump from Location #" .. i) end
			SetEntityHeading(AirSeaFuelZones[k].prop, heading)
			FreezeEntityPosition(AirSeaFuelZones[k].prop, 1)
	...
	if Config.TargetResource == 'ox_target' then
		...
		local modelOptions = {
			...
			[4] = {
				...
				canInteract = function()
					if Config.FuelDebug then
						print("Is Player In Special Fuel Zone?: " ..
							tostring(PlayerInSpecialFuelZone))
					end
					...
					if vehFuelLevel <= vehFuelShutoffLevel then
						if GetIsVehicleEngineRunning(vehPedIsIn) then
							...
							if Config.VehicleShutoffOnLowFuel['sounds']['enabled'] then
								...
								PlaySoundFromEntity(l_2613, "Landing_Tone", vehPedIsIn,
