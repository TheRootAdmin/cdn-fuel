if Config.PlayerOwnedGasStationsEnabled then -- This is so Player Owned Gas Stations are a Config Option, instead of forced. Set this option in shared/config.lua!
    ...
    local function SpawnGasStationPeds()
        ...
        for i = 1, #Config.GasStations do
            ...
            local ped = CreatePed(0, current.pedmodel, current.pedcoords.x, current.pedcoords.y, current.pedcoords.z,
                current.pedcoords.h, false, false)
    ...
    RegisterNetEvent('cdn-fuel:stations:updatelocation', function(updatedlocation)
        if Config.FuelDebug then
            if CurrentLocation == nil then CurrentLocation = 0 end
    ...
    RegisterNetEvent('cdn-fuel:stations:client:buyreserves', function(data)
        ...
        if Config.FuelDebug then
            print("^5Attempting Purchase of ^2" ..
                amount .. "^5 Fuel Reserves for location #" .. location .. "! Purchase Price: ^2" .. price)
        end
    end)
    ...
    RegisterNetEvent('cdn-fuel:station:client:initiatefuelpickup',
        function(amountBought, finalReserveAmountAfterPurchase, location)
            if amountBought and finalReserveAmountAfterPurchase and location then
                                    ...
                                    if IsPedInAnyVehicle(ped, false) then
                                        if GetVehiclePedIsIn(ped, false) == spawnedDeliveryTruck then
                                            ...
                                            if not alreadyHasTruck then
                                                local loc    = {}
                                                loc.x, loc.y = Config.GasStations[ReservePickupData.location].pedcoords
                                                    .x, Config.GasStations[ReservePickupData.location].pedcoords.y
                                                ...
                                                if not CreatedEventHandler then
                                                    ...
                                                    locationSwapHandler = AddEventHandler(
                                                        'cdn-fuel:stations:updatelocation', function(location)
                                                            if location == nil or location ~= ReservePickupData.location then
                                                                ...
                                                                if Config.Ox.DrawText then
                                                                    ...
                                                                    exports[Config.Core]:DrawText(
                                                                        Lang:t("draw_text_fuel_dropoff"), 'left')
                                                                end
                ...
                if Config.FuelDebug then
                    print(
                        "An error has occurred. The amountBought / finalReserveAmountAfterPurchase / location is nil: `cdn-fuel:station:client:initiatefuelpickup`")
                end
            end
        end)
    ...
    RegisterNetEvent('cdn-fuel:stations:client:purchasereserves:final',
        function(location, price, amount) -- Menu, seens after selecting the "purchase reserves" option.
            ...
            if CanOpen then
                if Config.FuelDebug then
                    print("Price: " .. price ..
                        "<br> Amount: " .. amount .. " <br> Location: " .. location)
                end
    ...
    RegisterNetEvent('cdn-fuel:stations:client:purchasereserves', function(data)
        ...
        QBCore.Functions.TriggerCallback('cdn-fuel:server:isowner', function(result)
            ...
            if result then
                ...
                QBCore.Functions.Notify(Lang:t("station_not_owner"), 'error', 7500)
                if Config.FuelDebug then
                    print("The Location: " .. CurrentLocation .. " is not owned by ID: " ..
                        CitizenID)
                end
                CanOpen = false
            end
        ...
        if CanOpen then
            ...
            if Config.Ox.Input then
                local reserves = lib.inputDialog('Purchase Reserves', {
                    ...
                    {
                        type    = "slider",
                        label   = 'Full Reserve Cost: $' ..
                            math.ceil(GlobalTax((Config.MaxFuelReserves - Currentreserveamount) *
                                    Config.FuelReservesPrice) +
                                ((Config.MaxFuelReserves - Currentreserveamount) * Config.FuelReservesPrice)) .. '',
                        default = Config.MaxFuelReserves - Currentreserveamount,
                        min     = 0,
                ...
                if reserves then
                    ...
                    if not reservesAmount then
                        QBCore.Functions.Notify(Lang:t("station_amount_invalid"), 'error', 7500)
                        return
                    end
                    Reservebuyamount = tonumber(reservesAmount)
                    if Reservebuyamount < 1 then
                        QBCore.Functions.Notify(Lang:t("station_more_than_one"), 'error', 7500)
                        return
                    end
                ...
                local reserves = exports['qb-input']:ShowInput({
                    header     = Lang:t("input_purchase_reserves_header_1") ..
                        Lang:t("input_purchase_reserves_header_2") ..
                        Currentreserveamount .. Lang:t("input_purchase_reserves_header_3") ..
                        math.ceil(GlobalTax((Config.MaxFuelReserves - Currentreserveamount) * Config.FuelReservesPrice) +
                            ((Config.MaxFuelReserves - Currentreserveamount) * Config.FuelReservesPrice)) .. "",
        ...
        if CanOpen then
            ...
            if Config.Ox.Input then
                local fuelprice = lib.inputDialog('Fuel Prices', {
                    ...
                    {
                        type    = "number",
                        label   = 'Enter New Fuel Price Per Liter',
                ...
                if fuelprice then
                    ...
                    if NewFuelPrice < Config.MinimumFuelPrice then
                        QBCore.Functions.Notify(Lang:t("station_price_too_low"), 'error', 7500)
                        return
                    end
                ...
                local fuelprice = exports['qb-input']:ShowInput({
                    header     = Lang:t("input_alter_fuel_price_header_1") ..
                        StationFuelPrice .. Lang:t("input_alter_fuel_price_header_2"),
    ...
    RegisterNetEvent('cdn-fuel:stations:client:sellstation:menu',
        function(data) -- Menu, seen after selecting the Sell this Location option.
            ...
            QBCore.Functions.TriggerCallback('cdn-fuel:server:isowner', function(result)
                if result then
                    if Config.FuelDebug then
                        print("The Location: " .. CurrentLocation .. " is owned by ID: " ..
                            CitizenID)
                    end
                    CanOpen = true
                else
                    ...
                    if Config.FuelDebug then
                        print("The Location: " ..
                            CurrentLocation .. " is not owned by ID: " .. CitizenID)
                    end
                    CanOpen = false
                end
            end, CurrentLocation)
                ...
                if Config.Ox.Input then
                    ...
                    if NewName then
                        ...
                        if not NewNameName then
                            QBCore.Functions.Notify(Lang:t("station_name_invalid"), 'error', 7500)
                            return
                        end
                        NewName = NewNameName
                        if type(NewName) ~= "string" then
                            QBCore.Functions.Notify(Lang:t("station_name_invalid"), 'error')
                            return
                        end
                        ...
                        if string.len(NewName) > Config.NameChangeMaxChar then
                            QBCore.Functions.Notify(Lang:t("station_name_too_long"), 'error')
                            return
                        end
                        if string.len(NewName) < Config.NameChangeMinChar then
                            QBCore.Functions.Notify(Lang:t("station_name_too_short"), 'error')
                            return
                        end
                        Wait(100)
                    ...
                    local NewName = exports['qb-input']:ShowInput({
                        header     = Lang:t("input_change_name_header_1") ..
                            Config.GasStations[CurrentLocation].label .. Lang:t("input_change_name_header_2"),
                ...
                if Config.Ox.Menu then
                    ...
                    exports['qb-menu']:openMenu({
                        ...
                        },
                        {
                            ...
                            isMenuHeader = true,
                            txt          = ReserveLevels ..
                                Lang:t("menu_manage_reserves_footer_1") ..
                                Config.MaxFuelReserves .. Lang:t("menu_manage_reserves_footer_2"),
                        },
                        {
                            header   = Lang:t("menu_manage_purchase_reserves_header"),
                            icon     = "fas fa-usd",
                            txt      = Lang:t("menu_manage_purchase_reserves_footer") ..
                                Config.FuelReservesPrice .. Lang:t("menu_manage_purchase_reserves_footer_2"),
                            params   = {
                                event = "cdn-fuel:stations:client:purchasereserves",
                                args  = {
                                    location = location,
                        ...
                        },
                        {
                            ...
                            icon     = "fas fa-usd",
                            txt      = "I want to change the price of fuel at my Gas Station! <br> Currently, it is $" ..
                                StationFuelPrice .. Lang:t("input_alter_fuel_price_header_2"),
                            params   = {
                                event = "cdn-fuel:stations:client:changefuelprice",
                                args  = {
                        ...
                        {
                            header = Lang:t("menu_sell_station_header_accept"),
                            txt    = Lang:t("menu_manage_sell_station_footer") ..
                                math.percent(Config.GasStationSellPercentage, GasStationCost),
                            icon   = "fas fa-usd",
                            params = {
                                event = "cdn-fuel:stations:client:sellstation:menu",
                                args  = {
        ...
        function(location) -- Menu, seen after selecting the Manage this Location Option.
            ...
            if CanOpen then
                if Config.Ox.Menu then
                    lib.registerContext({
                        id      = 'managefundsmenu',
                        title   = Lang:t("menu_manage_company_funds_header_2") .. Config.GasStations[CurrentLocation]
                            .label,
                        options = {
                            {
                    ...
                    exports['qb-menu']:openMenu({
                        {
                            header       = Lang:t("menu_manage_company_funds_header_2") ..
                                Config.GasStations[CurrentLocation].label,
                            isMenuHeader = true,
                            icon         = "fas fa-gas-pump",
                        },
                        {
        ...
        if CanOpen then
            ...
            if Config.Ox.Input then
                ...
                if Withdraw then
                    ...
                    if amount < 1 then
                        QBCore.Functions.Notify(Lang:t("station_withdraw_too_little"), 'error', 7500)
                        return
                    end
        ...
        if CanOpen then
            ...
            if Config.Ox.Input then
                ...
                if Deposit then
                    ...
                    if amount < 1 then
                        QBCore.Functions.Notify(Lang:t("station_deposit_too_little"), 'error', 7500)
                        return
                    end
    ...
    RegisterNetEvent('cdn-fuel:stations:client:purchasemenu',
        function(location) -- Menu, seen after selecting the purchase this location option.
            local bankmoney     = QBCore.Functions.GetPlayerData().money['bank']
            ...
            if bankmoney < costofstation then
                QBCore.Functions.Notify(Lang:t("not_enough_money_in_bank") .. " $" .. costofstation, 'error', 7500)
                return
            end
                ...
                exports['qb-menu']:openMenu({
                    ...
                    },
                    {
                        ...
                        txt          = Lang:t("menu_purchase_station_header_1") ..
                            costofstation .. Lang:t("menu_purchase_station_header_2"),
                    },
                    {
    ...
    RegisterNetEvent('cdn-fuel:stations:openmenu', function() -- Menu #1, the first menu you see.
        ...
        if Config.EmergencyShutOff then
            QBCore.Functions.TriggerCallback('cdn-fuel:server:checkshutoff', function(result)
                ...
                if Config.FuelDebug then
                    print("The result from Callback: Config.GasStations[" ..
                        CurrentLocation .. "].shutoff  = " .. PumpState)
                end
            end, CurrentLocation)
        else
            PumpState       = "enabled."
